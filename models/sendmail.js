...
module.exports = function (options) {
      ...
      resolveMx(domain, function (err, data) {
        ...
        function tryConnect (i) {
          ...
          sock.on('connect', function () {
            ...
            sock.end();
            //return mailserver that responds to transporter
            callback(null, data[i].exchange);
          });
        }
  ...
  function sendToSMTP (domain, envelope, body, cb) {
    const callback = (typeof cb === 'function') ? cb : function () {};
    connectMx(domain, function (err, destHost) {
      if (err) {
      ...
      connection.connect(()=>{
        connection.send(envelope, body, (err, info)=>{
            if(err){
                //Do nothing
                cb(new Error(err));
                return;
  ...
  function sendmail (mail, callback) {
    ...
    mailMe.build(function (err, message) {
      ...
      //Parse Message for transport
      simpleParser(message, (err, msg)=>{
        if(err) {
            logger.error('Error on creating message : ', err);
            callback(err, null);
            return; 
        ...
        envelope.from = msg.from.text;
        envelope.to = msg.to.text;

        for (let domain in groups) {
          sendToSMTP(domain, envelope, message, callback);
        }
      });
    });
  }
